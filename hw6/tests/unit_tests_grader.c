...
TestSuite(grade_output, .timeout=TEST_TIMEOUT, .disabled=false); // output file
...
/*Grader Series*/
Test(grade_output, simple_grade01, .description="Perform a simple replacement over entire file. Terms have the same length.") {
    char *test_name = "simple_grade01";
    prep_files("brown.txt", test_name);
    sprintf(args, "-s fox -r WOLFIE %s/%s.in.txt %s/%s.out.txt",
            TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(grade_output, simple_grade02, .description="Whole word match at start of lines only.") {
    char *test_name = "simple_grade02";
    prep_files("brown.txt", test_name);
    sprintf(args, "-s The -r START %s/%s.in.txt %s/%s.out.txt",
            TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(grade_output, simple_grade03, .description="Whole word match inside with punctuation boundaries.") {
    char *test_name = "simple_grade03";
    prep_files("brown.txt", test_name);
    sprintf(args, "-s end -r FINISH %s/%s.in.txt %s/%s.out.txt",
            TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(grade_output, simple_grade04, .description="Whole word match case-sensitive (does not match THE).") {
    char *test_name = "simple_grade04";
    prep_files("brown.txt", test_name);
    sprintf(args, "-s The -r FOUND %s/%s.in.txt %s/%s.out.txt",
            TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}

Test(grade_output, simple_grade05, .description="Whole word match does not replace inside longer words.") {
    char *test_name = "simple_grade05";
    prep_files("brown.txt", test_name);
    sprintf(args, "-s ZRTc -r XX %s/%s.in.txt %s/%s.out.txt",
            TEST_INPUT_DIR, test_name, TEST_OUTPUT_DIR, test_name);
    run_using_system_no_valgrind(test_name, args);
    expect_outfile_matches(test_name);
}
